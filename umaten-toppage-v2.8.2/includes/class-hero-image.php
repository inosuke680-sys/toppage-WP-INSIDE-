<?php
/**
 * ヒーロー画像自動設定クラス (v2.8.2)
 *
 * 本文から画像を抽出し、WordPressのアイキャッチ画像（_thumbnail_id）として自動設定します。
 * 記事ページでは本文中の画像を使用し、一覧ページではアイキャッチを表示します。
 *
 * v2.8.2: ヒーロー画像をアイキャッチに自動設定
 * - 本文の画像からattachment IDを取得してset_post_thumbnail()で設定
 * - 記事ページではアイキャッチを非表示（重複回避）
 * - v2.8.0の安定性を維持したシンプル実装
...
class Umaten_Toppage_Hero_Image {
        ...
        if ($image_url) {
            ...
            if ($attachment_id) {
                ...
                if (defined('WP_DEBUG') && WP_DEBUG) {
                    error_log("Umaten Toppage v2.8.2: Set thumbnail for post {$post_id}: attachment ID {$attachment_id}");
                }
            }
            ...
            // デバッグログ
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log("Umaten Toppage v2.8.2: Saved hero image URL for post {$post_id}: {$image_url}");
            }
    ...
    /**
     * 記事ページではアイキャッチを非表示（本文に画像があるため重複回避）
     */
