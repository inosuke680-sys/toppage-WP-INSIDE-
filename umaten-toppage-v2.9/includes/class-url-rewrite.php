...
class Umaten_Toppage_URL_Rewrite {
    ...
    public function handle_404_redirect() {
        ...
        // 【v2.9.0改善】デバッグログ - リクエスト情報を記録
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log("Umaten Toppage v2.9.0: Processing URL - Parts: " . count($parts) . ", Parent: '{$parent_slug}', Child: '{$child_slug}', Tag: '{$tag_slug}'");
        }

        // 【v2.9.0重要】まずタグとカテゴリの存在を確認
        ...
        // デバッグログ - タグ・カテゴリ情報
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log("Umaten Toppage v2.9.0: Terms found - Parent: " . ($parent_term ? $parent_term->name : 'none') . ", Child: " . ($child_term ? $child_term->name : 'none') . ", Tag: " . ($tag_term ? $tag_term->name : 'none'));
        }

        // 【v2.9.0完全修正】2段階URL（/親/子/）の場合
        if (count($parts) == 2) {
            // 【v2.9.0デバッグ】2段階URLの処理開始
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log("Umaten Toppage v2.9.0: 2-stage URL detected - Parent: '{$parent_slug}', Child: '{$child_slug}'");
            }

            // 子カテゴリが存在する場合はアーカイブページとして処理
            if ($child_term) {
                if (defined('WP_DEBUG') && WP_DEBUG) {
                    error_log("Umaten Toppage v2.9.0: Child category '{$child_slug}' exists (ID: {$child_term->term_id}, Name: '{$child_term->name}') - displaying as archive");
                }
            ...
            // 子カテゴリが存在しない場合の詳細ログ
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log("Umaten Toppage v2.9.0: Child category '{$child_slug}' NOT found - checking for post");
            }
            ...
            if ($post) {
                if (defined('WP_DEBUG') && WP_DEBUG) {
                    error_log("Umaten Toppage v2.9.0: Found post by slug '{$child_slug}' (ID: {$post->ID}, Title: '{$post->post_title}') - displaying as single post");
                }
            ...
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log("Umaten Toppage v2.9.0: No category or post found for '{$child_slug}' - returning 404");
            }
            return;
        }

        // 【v2.9.0完全修正】3段階URL（/親/子/第3セグメント/）の場合
        if (count($parts) == 3) {
            // 【重要】タグが存在する場合は、投稿として検索しない（アーカイブページとして処理）
            if ($tag_term) {
                if (defined('WP_DEBUG') && WP_DEBUG) {
                    error_log("Umaten Toppage v2.9.0: Tag '{$tag_slug}' exists (ID: {$tag_term->term_id}, Name: '{$tag_term->name}') - displaying as tag archive, NOT checking for post");
                }
            ...
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log("Umaten Toppage v2.9.0: No tag found for '{$tag_slug}', checking for post");
            }

            $post = $this->find_post_by_slug($tag_slug);
            if ($post) {
                if (defined('WP_DEBUG') && WP_DEBUG) {
                    error_log("Umaten Toppage v2.9.0: Found post by slug '{$tag_slug}' (ID: {$post->ID}, Title: '{$post->post_title}') - displaying as single post");
                }
                $this->setup_single_post_query($post);
                return;
            }
            ...
            // タグも投稿も見つからない場合
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log("Umaten Toppage v2.9.0: No tag or post found for '{$tag_slug}' - checking for category archive");
            }
            ...
            // 何も見つからない場合は404
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log("Umaten Toppage v2.9.0: No terms or posts found - returning 404");
            }
            return;
        }
    }
    ...
    /**
     * 【v2.9.0改善】投稿スラッグから投稿を検索（エラーハンドリング強化）
     *
    ...
    private function find_post_by_slug($slug) {
        ...
        if ($wpdb->last_error) {
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log("Umaten Toppage v2.9.0 DB Error in find_post_by_slug: " . $wpdb->last_error);
            }
            return null;
        }
        ...
        if ($post && defined('WP_DEBUG') && WP_DEBUG) {
            error_log("Umaten Toppage v2.9.0: find_post_by_slug('{$slug}') found post ID {$post->ID} (Title: '{$post->post_title}')");
        }
    ...
    /**
     * 【v2.9.0改善版】投稿ページとして表示するクエリをセットアップ
     *
    ...
    private function setup_single_post_query($post) {
        ...
        // デバッグログ
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log("Umaten Toppage v2.9.0: Setting up single post query for post ID " . $post->ID . " (Title: '{$post->post_title}', Slug: '{$post->post_name}')");
        }
    ...
    /**
     * 【v2.9.0改善版】アーカイブクエリをセットアップ（デバッグログ強化）
     */
    private function setup_archive_query($parent_term, $child_term, $tag_term) {
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            ...
            error_log("Umaten Toppage v2.9.0: Setting up archive query - Parent: {$parent_name}, Child: {$child_name}, Tag: {$tag_name}");
        }
