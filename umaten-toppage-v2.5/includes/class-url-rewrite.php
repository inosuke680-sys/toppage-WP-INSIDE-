<?php
/**
 * URLリライトルールクラス (v2.5.0 REST API/AJAX完全セーフ実装)
 * 投稿を検出したら積極的に表示する（管理画面・REST API・AJAXでは無効）
 */
...
class Umaten_Toppage_URL_Rewrite {
    ...
    public function handle_404_redirect() {
        ...
        // 【v2.5.0】投稿の存在を厳密にチェックし、見つかったら表示する
        // 2段階URL（/親/投稿名/）の場合
        if (count($parts) == 2) {
    ...
    /**
     * 【v2.5.0 改善版】投稿ページとして表示するクエリをセットアップ
     *
    ...
    private function setup_single_post_query($post) {
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log("Umaten Toppage v2.5.0: Setting up single post query for post ID " . $post->ID);
        }
