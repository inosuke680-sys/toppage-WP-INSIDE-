...
##############################################################################
# Umaten トップページ プラグイン v2.10.0 本番環境デプロイスクリプト
#
# 使用方法:
...
PLUGIN_NAME="umaten-toppage-v2.10"
...
create_backup() {
    ...
    # v2.6.0のバックアップ
    if [ -d "$WP_PLUGINS_DIR/umaten-toppage-v2.6" ]; then
        cp -r "$WP_PLUGINS_DIR/umaten-toppage-v2.6" "$BACKUP_DIR/"
        log_success "v2.6.0 をバックアップしました: $BACKUP_DIR/umaten-toppage-v2.6"
    fi

    # v2.10.0が既に存在する場合もバックアップ
...
activate_plugin() {
    ...
    if [[ "$response" =~ ^[Yy]$ ]]; then
        if command -v wp &> /dev/null; then
            ...
            # v2.6.0を無効化
            wp plugin deactivate umaten-toppage-v2.6 --allow-root 2>/dev/null || true
...
main() {
    ...
    log_info "3. 「Umaten トップページ v2.6.0」を無効化（必要に応じて）"
    log_info "4. 「Umaten トップページ v2.10.0」を有効化"
    log_info "5. 設定 → Umaten トップページ で設定を確認"
    log_info "6. WP_DEBUGを有効にして動作確認（推奨）"
    log_info ""
    ...
    log_info "  → WP_DEBUGを有効にしてエラーログを確認"
    log_info ""
