<?php
/**
 * URLリライトルールクラス (v2.6.0 記事ページ表示完全修正版)
 * 投稿を検出したら必ず表示する（管理画面・REST API・AJAXでは無効）
 * v2.6.0改善点：カテゴリチェックを緩和し、投稿が見つかった場合は必ず表示
 */
...
class Umaten_Toppage_URL_Rewrite {
    ...
    public function handle_404_redirect() {
        ...
        // 【v2.6.0改善】投稿の存在を厳密にチェックし、見つかったら必ず表示する
        // 2段階URL（/親/投稿名/）の場合
        if (count($parts) == 2) {
            $post = $this->find_post_by_slug($child_slug);
            if ($post) {
                // 【v2.6.0改善】投稿が見つかった場合は必ず表示（カテゴリチェックは緩和）
                $parent_category = get_term_by('slug', $parent_slug, 'category');

                // カテゴリが存在し、投稿がそのカテゴリに属している場合、または
                // カテゴリチェックを緩和して、投稿が見つかった場合は必ず表示
                if (!$parent_category || has_category($parent_category->term_id, $post)) {
                    // 【v2.6.0】投稿として積極的に表示
        ...
        if (count($parts) == 3) {
            ...
            if ($post) {
                ...
                // カテゴリが存在し、投稿がそのカテゴリに属している場合、または
                // カテゴリチェックを緩和して、投稿が見つかった場合は必ず表示
                if (!$target_cat || has_category($target_cat->term_id, $post)) {
                    // 【v2.6.0】投稿として積極的に表示
                    $this->setup_single_post_query($post);
                    return;
                }
    ...
    /**
     * 【v2.6.0 改善版】投稿ページとして表示するクエリをセットアップ
     *
    ...
    private function setup_single_post_query($post) {
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log("Umaten Toppage v2.6.0: Setting up single post query for post ID " . $post->ID . " (Title: " . $post->post_title . ")");
        }
