...
##############################################################################
# Umaten トップページ プラグイン v2.8.2 本番環境デプロイスクリプト
#
# 使用方法:
...
PLUGIN_NAME="umaten-toppage-v2.8.2"
...
main() {
    ...
    log_info "4. 「Umaten トップページ v2.8.2」を有効化"
    log_info "5. 設定 → パーマリンク → 「変更を保存」（重要！）"
    log_info "6. 既存投稿のアイキャッチ画像を一括設定（推奨）："
    log_info "   cd /home/kusanagi/.../DocumentRoot && \\"
    log_info "   curl -o bulk-set-hero-images-v2.8.2.php ${GITHUB_RAW_URL}/bulk-set-hero-images-v2.8.2.php && \\"
    log_info "   php bulk-set-hero-images-v2.8.2.php"
    log_info ""
    log_info "v2.8.2の新機能："
    log_info "- ヒーロー画像を自動的にWordPressアイキャッチとして設定"
    log_info "- 記事ページではアイキャッチを非表示（重複回避）"
