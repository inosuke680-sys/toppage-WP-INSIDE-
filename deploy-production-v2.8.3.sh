...
##############################################################################
# Umaten トップページ プラグイン v2.8.3 本番環境デプロイスクリプト
#
# 使用方法:
...
PLUGIN_NAME="umaten-toppage-v2.8.3"
...
detect_wordpress_path() {
    ...
    if [ -d "/home/kusanagi" ]; then
        log_info "Kusanagi環境を検出しました"

        # Kusanagiサイトディレクトリを検索
        for site_dir in /home/kusanagi/*/DocumentRoot/wp-content/plugins; do
            if [ -d "$site_dir" ]; then
                WP_PLUGINS_DIR="$site_dir"
                log_success "Kusanagi WordPressプラグインディレクトリを検出: $WP_PLUGINS_DIR"
                return 0
            fi
        done
    fi
...
create_backup() {
    ...
    # v2.8.1のバックアップ
    if [ -d "$WP_PLUGINS_DIR/umaten-toppage-v2.8.1" ]; then
        cp -r "$WP_PLUGINS_DIR/umaten-toppage-v2.8.1" "$BACKUP_DIR/"
        log_success "v2.8.1 をバックアップしました: $BACKUP_DIR/umaten-toppage-v2.8.1"
    fi

    # v2.8.3が既に存在する場合もバックアップ
...
install_plugin() {
    ...
    # パーミッションの設定（Kusanagi環境対応）
    if id -u kusanagi &>/dev/null; then
        log_info "Kusanagi環境を検出 - kusanagi:www でパーミッション設定"
        chown -R kusanagi:www "$WP_PLUGINS_DIR/$PLUGIN_NAME"
    elif id -u www-data &>/dev/null; then
        log_info "www-data ユーザーでパーミッション設定"
        chown -R www-data:www-data "$WP_PLUGINS_DIR/$PLUGIN_NAME"
    elif id -u nginx &>/dev/null; then
        log_info "nginx ユーザーでパーミッション設定"
        chown -R nginx:nginx "$WP_PLUGINS_DIR/$PLUGIN_NAME"
    else
        log_warning "適切なWebサーバーユーザーが見つかりません - 現在のユーザーのまま続行"
    fi
...
activate_plugin() {
    ...
    if [[ "$response" =~ ^[Yy]$ ]]; then
        if command -v wp &> /dev/null; then
            # WordPress ルートディレクトリを検出
            WP_ROOT=$(dirname $(dirname "$WP_PLUGINS_DIR"))
            cd "$WP_ROOT" || exit 1

            log_info "WordPressルートディレクトリ: $WP_ROOT"

            # v2.8.1を無効化
            wp plugin deactivate umaten-toppage-v2.8.1 --allow-root 2>/dev/null || true

            # v2.8.3を有効化
            wp plugin activate "$PLUGIN_NAME" --allow-root || {
                log_error "プラグインの有効化に失敗しました"
                log_info "手動で有効化してください"
                return 1
            }
...
main() {
    ...
    log_info "3. 「Umaten トップページ v2.8.1」を無効化（必要に応じて）"
    log_info "4. 「Umaten トップページ v2.8.3」を有効化"
    log_info "5. 設定 → パーマリンク → 「変更を保存」（重要！）"
    log_info "6. 既存投稿のアイキャッチ画像を一括設定（推奨）："
    log_info "   cd /home/kusanagi/.../DocumentRoot && \\"
    log_info "   curl -o bulk-set-hero-images-v2.8.3.php ${GITHUB_RAW_URL}/bulk-set-hero-images-v2.8.3.php && \\"
    log_info "   php bulk-set-hero-images-v2.8.3.php"
    log_info ""
    log_info "v2.8.3の新機能（SEO最適化版）："
    log_info "- アイキャッチ設定でSEO効果を最大化（OGP、Schema.org対応）"
    log_info "- 記事ページでは3段階フィルターでアイキャッチを完全非表示（重複回避）"
    log_info "- SWELLテーマ完全対応（どんな取得方法でもブロック）"
    log_info "- 一覧ページでアイキャッチが正常に表示される"
    log_info ""
    log_info "トラブルシューティング："
    log_info "- 一覧ページでヒーロー画像が表示されない場合："
    log_info "  → ブラウザとWordPressのキャッシュをクリア"
    log_info "  → 一括設定スクリプトを実行"
    log_info "- 記事ページが404になる場合："
    log_info "  → パーマリンク設定を保存し直す"
    log_info ""
