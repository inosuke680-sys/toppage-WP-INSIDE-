<?php
/**
 * URLリライトルールクラス (v2.10.0 完全版)
 * 投稿とタグを正しく識別して適切なページを表示（管理画面・REST API・AJAXでは無効）
 *
 * v2.10.0改善：
 * - v2.8.0のタグ・投稿判定ロジックを維持
 * - 2段階URL処理のデバッグログを大幅強化
 * - 「すべてのジャンル」ボタンの遷移問題を調査・修正
 * - アーカイブページの表示処理を最適化
 *
 * v2.8.0完全修正：
...
class Umaten_Toppage_URL_Rewrite {
    ...
    private function __construct() {
        // 【v2.10.0重要】404時のみカスタム処理を実行（優先度を1に設定 - 最優先実行）
        add_action('template_redirect', array($this, 'handle_404_redirect'), 1);

        // 【v2.10.0新機能】WordPressの自動リダイレクトを防止
    ...
    public function prevent_canonical_redirect($redirect_url, $requested_url) {
        // カスタムURL処理中はリダイレクトを無効化
        if (!is_404() && (is_archive() || is_single())) {
            // デバッグログ
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log("Umaten Toppage v2.10.0: Preventing canonical redirect");
            }
            return false;
        }
        return $redirect_url;
    ...
    public function handle_404_redirect() {
        ...
        // 【v2.10.0改善】デバッグログ - リクエスト情報を記録
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log("Umaten Toppage v2.10.0: Processing URL - Parts: " . count($parts) . ", Parent: '{$parent_slug}', Child: '{$child_slug}', Tag: '{$tag_slug}'");
        }

        // 【v2.10.0重要】まずタグとカテゴリの存在を確認
        ...
        // デバッグログ - タグ・カテゴリ情報
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log("Umaten Toppage v2.10.0: Terms found - Parent: " . ($parent_term ? $parent_term->name : 'none') . ", Child: " . ($child_term ? $child_term->name : 'none') . ", Tag: " . ($tag_term ? $tag_term->name : 'none'));
        }

        // 【v2.10.0完全修正】2段階URL（/親/子/）の場合
        if (count($parts) == 2) {
            // 【v2.10.0デバッグ】2段階URLの処理開始
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log("Umaten Toppage v2.10.0: 2-stage URL detected - Parent: '{$parent_slug}', Child: '{$child_slug}'");
            }

            // 子カテゴリが存在する場合はアーカイブページとして処理
            if ($child_term) {
                if (defined('WP_DEBUG') && WP_DEBUG) {
                    error_log("Umaten Toppage v2.10.0: Child category '{$child_slug}' exists (ID: {$child_term->term_id}, Name: '{$child_term->name}') - displaying as archive");
                }
            ...
            // 子カテゴリが存在しない場合の詳細ログ
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log("Umaten Toppage v2.10.0: Child category '{$child_slug}' NOT found - checking for post");
            }
            ...
            if ($post) {
                if (defined('WP_DEBUG') && WP_DEBUG) {
                    error_log("Umaten Toppage v2.10.0: Found post by slug '{$child_slug}' (ID: {$post->ID}, Title: '{$post->post_title}') - displaying as single post");
                }
            ...
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log("Umaten Toppage v2.10.0: No category or post found for '{$child_slug}' - returning 404");
            }
            return;
        }

        // 【v2.10.0完全修正】3段階URL（/親/子/第3セグメント/）の場合
        if (count($parts) == 3) {
            // 【重要】タグが存在する場合は、投稿として検索しない（アーカイブページとして処理）
            if ($tag_term) {
                if (defined('WP_DEBUG') && WP_DEBUG) {
                    error_log("Umaten Toppage v2.10.0: Tag '{$tag_slug}' exists (ID: {$tag_term->term_id}, Name: '{$tag_term->name}') - displaying as tag archive, NOT checking for post");
                }
            ...
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log("Umaten Toppage v2.10.0: No tag found for '{$tag_slug}', checking for post");
            }

            $post = $this->find_post_by_slug($tag_slug);
            if ($post) {
                if (defined('WP_DEBUG') && WP_DEBUG) {
                    error_log("Umaten Toppage v2.10.0: Found post by slug '{$tag_slug}' (ID: {$post->ID}, Title: '{$post->post_title}') - displaying as single post");
                }
                $this->setup_single_post_query($post);
                return;
            }
            ...
            // タグも投稿も見つからない場合
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log("Umaten Toppage v2.10.0: No tag or post found for '{$tag_slug}' - checking for category archive");
            }
            ...
            // 何も見つからない場合は404
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log("Umaten Toppage v2.10.0: No terms or posts found - returning 404");
            }
            return;
        }
    }
    ...
    /**
     * 【v2.10.0改善】投稿スラッグから投稿を検索（エラーハンドリング強化）
     *
    ...
    private function find_post_by_slug($slug) {
        ...
        if ($wpdb->last_error) {
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log("Umaten Toppage v2.10.0 DB Error in find_post_by_slug: " . $wpdb->last_error);
            }
            return null;
        }
        ...
        if ($post && defined('WP_DEBUG') && WP_DEBUG) {
            error_log("Umaten Toppage v2.10.0: find_post_by_slug('{$slug}') found post ID {$post->ID} (Title: '{$post->post_title}')");
        }
    ...
    /**
     * 【v2.10.0改善版】投稿ページとして表示するクエリをセットアップ
     *
    ...
    private function setup_single_post_query($post) {
        ...
        // デバッグログ
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log("Umaten Toppage v2.10.0: Setting up single post query for post ID " . $post->ID . " (Title: '{$post->post_title}', Slug: '{$post->post_name}')");
        }
    ...
    /**
     * 【v2.10.0改善版】アーカイブクエリをセットアップ（デバッグログ強化）
     */
    private function setup_archive_query($parent_term, $child_term, $tag_term) {
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            ...
            error_log("Umaten Toppage v2.10.0: Setting up archive query - Parent: {$parent_name}, Child: {$child_name}, Tag: {$tag_name}");
        }
